...
app.whenReady().then(() => {
  ...
  ipcMain.handle("syncConfig", handleSyncConfig);
...
/**
 * Get task configuration from environment, if no file, use default task config file
 * @param {Event} event The Electron renderer event
 * @param {string} studyID The study id
 * @param {string} participantID The participant id
 * @returns {Object} task config object
 */
function handleSyncConfig(event, studyID, participantID) {
  let config;
  try {
    ...
    config = JSON.parse(fs.readFileSync(path.resolve(basePath, "config/defaultTaskConfig.json")));
  }
  return config;
}
...
function handleGetCredentials() {
  ...
  const robotID = process.env.REACT_APP_ROBOT_ID;

  if (studyID) log.info("Received study from ENV: ", studyID);
  if (participantID) log.info("Received participant from ENV: ", participantID);
  if (robotID) log.info("Received robot from ENV: ", robotID);
  return { studyID, participantID, robotID };
}
...
/**
 * Helper function to get the configuration path
 */
